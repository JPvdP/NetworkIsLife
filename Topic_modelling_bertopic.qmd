...
```{r, eval = FALSE}
#| eval: false
hier_topics = topic_model_scopus.hierarchical_topics(data_keep)
topic_model_scopus.visualize_hierarchical_documents(data_keep, hier_topics)
```

```{r, eval = FALSE}
#| eval: false
topic_model_scopus.visualize_hierarchy(hierarchical_topics=hier_topics)
```


```{r, eval = FALSE}
#| eval: false
# Assuming `documents` is your list of documents and you have already fitted a BERTopic model:
...
topic_words_df.to_csv("top_words_per_topic_crunchbase.csv", index=False)
```

```{r, eval = FALSE}
#| eval: false
topic_model_scopus.visualize_barchart(topics=[1,2,3,4,5,6,7,8])
```
...
```{r, eval = FALSE}
#| eval: false
plop = topic_model_scopus.get_document_info(data_keep)
plop.to_csv('CE_topics.csv', index=False)
```
